...
class DataExporter:
    ...
    def export_date_range(
        ...
        try:
            articles = DataExporter.get_articles_query(
                db, start_date=start_date, end_date=end_date, source=source, limit=limit
            ).all()
    ...
    def get_export_stats() -> Dict[str, Any]:
        ...
        try:
            ...
            # Articles by source
            source_stats = (
                db.query(FinancialNews.source, func.count(FinancialNews.id).label("count"))
                .group_by(FinancialNews.source)
                .all()
            )
            ...
            recent_articles = (
                ...
                .filter(FinancialNews.published_date >= seven_days_ago)
                .count()
            )
            ...
            return {
                ...
                "articles_by_source": {str(source): int(count) for source, count in source_stats},
