...
from .models import APIUsage, FinancialNews, ScrapingLog
...
async def get_articles(
    ...
    try:
        ...
        articles = (
            query.order_by(FinancialNews.published_date.desc()).offset(skip).limit(limit).all()
        )
        ...
        for article in articles:
            response_articles.append(
                ArticleResponse(
                    ...
                    mentioned_stocks=(
                        json.loads(article.mentioned_stocks) if article.mentioned_stocks else []
                    ),
                    mentioned_companies=(
                        json.loads(article.mentioned_companies)
                        if article.mentioned_companies
                        else []
                    ),
                    mentioned_persons=(
                        json.loads(article.mentioned_persons) if article.mentioned_persons else []
                    ),
                    category=article.category,
                    subcategory=article.subcategory,
                    tags=json.loads(article.tags) if article.tags else [],
                    word_count=article.word_count or 0,
                    read_time_minutes=article.read_time_minutes or 0,
                )
            )
...
async def get_article(article_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        response_article = ArticleResponse(
            ...
            mentioned_companies=(
                json.loads(article.mentioned_companies) if article.mentioned_companies else []
            ),
            mentioned_persons=(
                json.loads(article.mentioned_persons) if article.mentioned_persons else []
            ),
...
async def search_articles(search_request: SearchRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        articles = (
            query.order_by(FinancialNews.published_date.desc()).limit(search_request.limit).all()
        )
...
async def export_data(export_request: ExportRequest):
    ...
    try:
        ...
        start_str = export_request.start_date.strftime("%Y-%m-%d")
        end_str = export_request.end_date.strftime("%Y-%m-%d")
        filename = f"financial_news_{start_str}_to_{end_str}.{export_request.format}"
...
async def trigger_scraping(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return {"message": "Scraping started in background", "status": "started"}
...
async def get_scraping_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        articles_today = (
            db.query(FinancialNews).filter(FinancialNews.published_date >= today_start).count()
        )

        status = ScrapingStatusResponse(
            success=last_scrape.success if last_scrape else False,
            message=(
                "Last scraping completed successfully"
                if (last_scrape and last_scrape.success)
                else "No recent scraping"
            ),
